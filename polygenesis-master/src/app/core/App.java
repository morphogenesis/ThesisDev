...
public class App extends PApplet {
    ...
    private static app.core.VoronoiDiagram VSYS;
...
//	private Old_Metaball MBALL;
    ...
    public void setup() {
        ...
        ellipseMode(RADIUS);
        textAlign(LEFT);
        textFont(pfont = createFont("SourceCodePro", 10), 10);
        strokeWeight(1);
        noStroke();
        noFill();
        ...
        VSYS = new app.core.VoronoiDiagram(this);
        app.core.Gui.init();
...
//		MBALL.update();
        ...
        if (RECORDING) {
            RECORDING = false;
            endRecord();
            System.out.println("SVG EXPORTED SUCCESSFULLY");
        }
        CP5.draw();
    }
    ...
    public void keyPressed() {
        ...
        if (key == TAB) {
            app.core.Gui.isEditMode = !app.core.Gui.isEditMode;
        }
        if (app.core.Gui.isEditMode) {
            switch (key) {
                case '1':
                    app.core.Gui.drawVorPoly = !app.core.Gui.drawVorPoly;
                    break;
                case '2':
                    app.core.Gui.drawVorBez = !app.core.Gui.drawVorBez;
                    break;
                case '3':
                    app.core.Gui.drawVorVec = !app.core.Gui.drawVorVec;
                    break;
                case '4':
                    app.core.Gui.drawPhysInfo = !app.core.Gui.drawPhysInfo;
                    app.core.Gui.drawVorInfo = !app.core.Gui.drawVorInfo;
                    break;
                case 'a':
                    Editor.createNewNode(app.core.Gui.nameTextfield.getStringValue(), app.core.Gui.radiusSlider.getValue(), mousePos());
                    app.core.Gui.toggleObjProperties();
                    break;
                case 'f':
                    Editor.createNewEdge();
                    break;
                case 'x':
                    Editor.deleteNode();
                    break;
                case 'v':
                    ...
                    break;
                case 'q':
                    Editor.createNewBranch(app.core.Gui.capacitySlider.getValue(), true);
                    break;
                case 'w':
                    Editor.createNewBranch(app.core.Gui.capacitySlider.getValue(), false);
                    break;
                case 'l':
                    Editor.lockNode();
                    break;
                case 'm':
                    Old_Metaball.isMetaUpdating = !Old_Metaball.isMetaUpdating;
                    break;
...
//		if (e > 0) {System.out.println("-");} else if (e == 0) {System.out.println("0");} else if (e < 0) {System.out.println("+");}
    ...
    public void controlEvent(ControlEvent theEvent) {
        app.core.Gui.controlEvent(this, theEvent);
    }
}
